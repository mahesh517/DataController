...
import com.app.datacontorller.Utils.AppConst;
...
public class DataManager {
    ...
    public void requestPermission(Context context) {
        @SuppressLint("InlinedApi") Intent intent = new Intent(new Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS));
    ...
    public List<AppItem> getTargetAppTimeline(Context context, String target, int offset, long startTime, long endTime) {


        List<AppItem> items = new ArrayList<>();
        Log.e("Timings", startTime + "/" + endTime + "/" + target);
...
//
            ...
            if (startTime == 0 && endTime == 0) {
                ...
                events = manager.queryEvents(startTime, endTime);
            } else if (startTime == 0 && endTime > 0) {
                events = manager.queryEvents(range[0], endTime);
            } else if (startTime > 0 && endTime == 0) {
                events = manager.queryEvents(startTime, range[1]);
            }
    ...
    public List<AppItem> getApps(Context context, int sort, int offset) {
        ...
        if (manager != null) {
            ...
            Log.e("range", new Gson().toJson(range));
